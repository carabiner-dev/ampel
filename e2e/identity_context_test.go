...
// the workflow issuer and leaves the origin repo as a {{ .Context.source_repo }}
// template; the verifier supplies the repo at runtime. Resolution happens before
// CheckIdentities, AND-ed inside the identity, so it fails closed. Hits the
// sigstore trust root over TUF (network).
